...
//RETORNA FALSO SI NO EXISTE EL USUARIO SI DA TRUE ES QUE YA EXISTE
    ...
    //CONSULTA #4
    public ArrayList<User> sameProduct(String userName) {
        ArrayList<User> userList = new ArrayList<>();
        try (Session session = driver.session()) {
            Result result = session.run(
                    "MATCH (u:Usuario {usuario:$x})-[:COMPRA]->(p:Producto)-[:COMPRA]-(d:Usuario) RETURN d",
                    parameters("x", userName));
            while (result.hasNext()) {
                Record record = result.next();
                User user = new User(record.get(0).get("nombre").asString(), record.get(0).get("apellido").asString());
                userList.add(user);
            }
        }
        return userList;
